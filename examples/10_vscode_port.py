...
def draw_feasible_set(protein, feasible_set):
    # Draw the protein
...
def qaoa_layer(gamma, beta, H_cost, H_mixer):
    qaoa.cost_layer(gamma, H_cost)
    qaoa.mixer_layer(beta, H_mixer)
    
# Then repeat it in a circuit with starting in a super position of all bitstrings
def circuit(params, qubits):     # Gamma and Beta values can be put together to be an array of parameters
...
#@qml.qnode(dev)
#def average_cost(params, qubits, H_cost, dev):
#    circuit(params, qubits)
#    return qml.expval(H_cost)

# Obtains the probability for all the states
#@qml.qnode(dev)
#def get_probs(params, qubits):
#    circuit(params, qubits)
...
def optimization():
    ...
    return cost_vector, best_params_optimisation


def draw_optgraph(cost_vector):
...
def main():
    ...
    draw_feasible_set(protein, feasible_set)
    ...
    dev = qml.device('default.qubit', wires = qubits) #skapar simuleringen
    ...
    cost_vector, best_params = optimization()

    draw_optgraph(cost_vector)
